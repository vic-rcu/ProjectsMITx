...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    for i in range(n):
        res += hinge_loss_single(feature_matrix[i], labels[i], theta, theta_0)
...
def perceptron(feature_matrix, labels, T):
    ...
    """
    # Your code here
    n, dimension= feature_matrix.shape
    ...
    for i in range(T):
        indices = get_order(n)
        for j in indices:
            theta, theta_0 = perceptron_single_step_update(feature_matrix[j], labels[j], theta, theta_0)
...
def average_perceptron(feature_matrix, labels, T):
    ...
    # Your code here
    n, dimension = feature_matrix.shape
    current_theta = np.zeros(dimension)
    current_theta_0 = 0
...
def classifier_accuracy(
    ...
    classification_train = classify(train_feature_matrix, theta, theta_0)
    classification_val = classify(val_feature_matrix, theta, theta_0)

    return accuracy(classification_train, train_labels), accuracy(classification_val, val_labels)
