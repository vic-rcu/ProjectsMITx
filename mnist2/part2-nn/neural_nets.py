...
class NeuralNetwork():
    ...
    def train(self, x1, x2, y):
        ...
        relu_der_vec = np.vectorize(rectified_linear_unit_derivative)

        output_vec = np.vectorize(output_layer_activation)
        output_der_vec = np.vectorize(output_layer_activation_derivative)
        ...
        output =  np.matmul(self.hidden_to_output_weights.T, hidden_layer_activation)
        activated_output = output_vec(output)

        ### Backpropagation ###

        # Compute gradients
        output_layer_error = (activated_output - y) * output_der_vec(output)
        hidden_layer_error = np.matmul(np.multiply(self.hidden_to_output_weights, relu_der_vec(
            hidden_layer_weighted_input)), output_layer_error)

        bias_gradients = hidden_layer_error
        hidden_to_output_weight_gradients = np.matmul(hidden_layer_activation, output_layer_error)
